...
//    exit;
    ...
    function plugins_check()
    {
        if (!class_exists('CPTP')) {
            ...
            ?>
            <div class="updated">
                <p><?php _e( 'Affiliator: Εγκαταστήστε και ενεργοποιήστε το <a href="plugin-install.php?tab=plugin-information&plugin=custom-post-type-permalinks&TB_iframe=true&width=772&height=865" class="thickbox open-plugin-details-modal">Custom Post Type Permalinks</a> plugin για καλύτερα αποτελέσματα!', 'affiliator' ); ?></p>
            </div>
            <?php
        }
            ...
            if (!in_array($fields['lw_product_id'], $passed)) {
                if (get_option('affiliator_auto_post_limit') && $k <= get_option('affiliator_auto_post_limit')) {
                    ...
                    if (!$this->affiliator_get_option('product_' . $fields['lw_product_id'], false)) {
                        ...
                        $post = array(
                            'ID' => '',
                            'post_content' => (trim($fields['description']))?$fields['description']:$fields['product_name'],
                        ...
                        foreach ($fields as $ka => $va) {
                            if (trim($ka) && trim($va)) {
                                add_post_meta($pid, strtolower($ka), $va);
                            }
                        }
    ...
    public function options_action()
    {
        ...
        $fields = array(
            'general_settings' => array(
                ...
                array(
                    'name' => 'lw_api_username',
                    'label' => __('Linkwise API Username', 'affiliator'),
                    'desc' => __('Το API username στην Linkwise', 'affiliator'),
                    'type' => 'text',
                    'default' => $this->affiliator_get_option('lw_api_username', '')
                ),
                array(
                    'name' => 'lw_api_password',
                    'label' => __('Linkwise API Password', 'affiliator'),
                    'desc' => __('Το API password στην Linkwise', 'affiliator'),
                    'type' => 'text',
                    'default' => $this->affiliator_get_option('lw_api_password', '')
                ),
//                array(
//                    'name' => 'fv_cd',
...
//                ),
    ...
    function  get_lw_progs(){

        if(!$programs = get_transient('lw_programs')){
            $programs = file_get_contents('https://'.$this->affiliator_get_option('lw_api_username').':'.$this->affiliator_get_option('lw_api_password').'@affiliate.linkwi.se/en/rest_programs.html?joined=yes&countries=Greece&format=json');
            $programs = json_decode($programs, 1);
            echo set_transient('lw_programs',$programs,3600 * 8);
        }
        return $programs;
    }
            ...
            <?php foreach ($programs as $k => $v):
                ...
                <tr>
                    ...
                    <td><?php echo $v['name'] ?><br/>
                        ...
                        <input type="hidden" name="feedurl"
                               value="https://affiliate.linkwi.se/feeds/<?php echo $this->affiliator_get_option('lw_cd', 'CD421') ?>/columns-lw_product_id,product_id,barcode,sku,part_number,isbn,model_name,product_name,description,category,brand_name,site_url,tracking_url,thumb_url,image_url,in_stock,availability,valid_from,valid_to,on_sale,currency,price,full_price,discount,city,times_bought,longitude,latitude,address,size,colour,program_id,program_name,custom/catinc-0/catex-0/proginc-<?php echo $v['id'] ?>/progex-0/feed.json"/>
                    </td>
    ...
    public function parse_feed()
    {
        ...
        $file = json_decode($file,1);
        $this->show_products_table($file, $page, $query, $category);
        exit;
    }
